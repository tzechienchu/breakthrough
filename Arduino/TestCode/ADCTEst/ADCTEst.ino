...
void loop()
{
  //for (int i=0;i<1000;i++) 
  {
    ...
    probeData(output3[0],output3[1],output3[2],output3[3]);
    //probeData(a1[index[0]],delta[0],output2[0],output3[0]);
    //probeData(a2[index[1]],delta[1],output2[1],output3[1]);//ADC1
...
//ADC
void probeData(int d1,int d2,int d3,int d4)
{ 
  unsigned long time;
  time = millis();
...
int sampleADC(int channel,int temp[],int tempout[])
{
  ...
  //8 Bits
  if (output1[channel] == 0x00) output2[channel] = 0;
  if (output1[channel] == 0xFF) output2[channel] = 1;  
